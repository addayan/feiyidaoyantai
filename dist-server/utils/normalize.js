...
export function normalizeGeneratedResult(raw, request) {
    ...
    if (!raw.story?.synopsis)
        warnings.push('story.synopsis 缺失，已生成兜底内容');
    // --- 角色 ---
    const characters = normalizeCharacters(raw.characters || [], request);
    if (!raw.characters || raw.characters.length === 0)
        warnings.push('characters 为空，已生成兜底角色');
    // --- 场景 ---
    const scenes = normalizeScenes(raw.scenes || [], request);
    if (!raw.scenes || raw.scenes.length === 0)
        warnings.push('scenes 为空，已生成兜底场景');
    // --- 镜头（必须为 8 个） ---
    const shots = normalizeShots(raw.shots || [], request, scenes);
    const originalShotCount = raw.shots?.length || 0;
    if (originalShotCount < 8)
        warnings.push(`shots 仅 ${originalShotCount} 个，已补齐到 8 个`);
    if (originalShotCount > 8)
        warnings.push(`shots 有 ${originalShotCount} 个，已截取前 8 个`);
    // --- 声音设计 ---
    const soundDesign = {
    ...
    if (!raw.soundDesign)
        warnings.push('soundDesign 为空，已生成兜底内容');
    // --- 文化表达检查 ---
    const cultureCheck = {
    ...
    if (!raw.cultureCheck)
        warnings.push('cultureCheck 为空，已生成兜底内容');
    // --- 参赛说明 ---
    ...
    if (!raw.submissionNote)
        warnings.push('submissionNote 为空，已生成兜底内容');
    // --- 发布文案 ---
    const socialPosts = {
    ...
    if (!raw.socialPosts)
        warnings.push('socialPosts 为空，已生成兜底内容');
    // --- 生成元信息 ---
...
// ===== 角色标准化 =====
function normalizeCharacters(chars, request) {
...
function getDefaultCharacterField(field, request) {
    const defaults = {
        name: '手艺人',
        age: '60岁',
...
function getDefaultCharacters(request) {
    return [
        {
            name: '小雅',
            age: '22岁',
...
// ===== 场景标准化 =====
function normalizeScenes(scenes, request) {
    ...
    const normalized = scenes.map(s => {
        for (const field of requiredFields) {
            if (!s[field] || s[field] === '') {
                s[field] = getDefaultSceneField(field, request);
            }
        }
        return s;
    });
...
function getDefaultSceneField(field, request) {
    const defaults = {
        name: '传统工坊',
        time: '白天',
...
function getDefaultScenes(request) {
    return [
        {
            name: '老宅工坊',
            time: '黄昏至夜晚',
...
// ===== 镜头标准化（必须 8 个）=====
function normalizeShots(shots, request, scenes) {
    ...
    const normalized = shots.map((s, i) => {
        ...
        for (const field of requiredFields) {
            if (s[field] === undefined || s[field] === null || s[field] === '') {
                s[field] = getDefaultShotField(field, request, i, scenes);
            }
        }
        // 确保 id 格式正确
        s.id = `shot-${i + 1}`;
...
function getDefaultShotField(field, request, index, scenes) {
...
function createDefaultShot(request, index, scenes) {
    const sceneName = scenes[index % scenes.length]?.name || '传统工坊';
    const shot = {
        id: `shot-${index + 1}`,
        scene: sceneName,
