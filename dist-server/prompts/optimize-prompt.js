...
const OPTIMIZE_TYPE_TIPS_FIELD = {
    ...
    generatability: '增强 AI 可生成性：简化到 1-2 个主体、1 个动作、保持环境一致，控制在 200-400 字。',
    simplify: '简化提示词：精简到核心关键词，去除冗余描述，控制在 150 字以内。',
    seedream: '适配 Seedream：使用 Seedream 推荐的提示词结构（主体 + 环境 + 光影 + 风格 + 质量），中英混合。',
    seedance: '适配 Seedance：使用 Seedance 推荐的提示词结构（场景 + 动作 + 运镜 + 时长 + 风格），强调动态描述。',
    custom: '自定义优化。',
};
/**
 * 构建单个提示词字段优化 prompt。
 */
export function buildOptimizePromptFieldPrompt(project, shotIndex, promptField, optimizeType, customInstruction) {
    ...
    const currentPrompt = shot ? shot[promptField] || '（当前为空）' : '（当前为空）';
