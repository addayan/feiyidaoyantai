...
import type { GenerateRequest, AIError } from '../types';
...
/** 创建 AIError 响应 */
export function aiErrorResponse(res: Response, code: AIError['code'], message: string, retryable: boolean, status: number) {
  const err: AIError = { code, message, retryable };
  return res.status(status).json({ error: err });
}

/** 调用火山方舟 API */
export async function callArkAPI(prompt: string): Promise<string> {
  ...
  if (!response.ok) {
    if (response.status === 429) {
      throw Object.assign(new Error('AI 请求被限流'), { code: 'AI_RATE_LIMITED' as const, retryable: true });
    }
    throw Object.assign(new Error(`AI 请求失败: HTTP ${response.status}`), { code: 'AI_REQUEST_FAILED' as const, retryable: response.status >= 500 });
  }
...
router.post('/api/generate-storyboard', async (req: Request, res: Response) => {
  ...
  const { heritageType, topic, purpose, duration, style }: GenerateRequest = req.body;
  ...
  const request: GenerateRequest = { heritageType, topic, purpose, duration, style };
